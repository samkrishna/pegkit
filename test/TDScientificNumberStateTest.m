...
- (void)test1e2 {
    ...
    r.string = s;
    PKToken *tok = [numberState nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    TDEquals(100.0, tok.doubleValue);
    TDTrue(tok.isNumber);
}
...
- (void)test2dot0e2 {
    ...
    r.string = s;
    PKToken *tok = [numberState nextTokenFromReader:r startingWith:[r read] tokenizer:t];
    TDEquals(200.0, tok.doubleValue);
    TDTrue(tok.isNumber);
...
- (void)testHexAlpha {
    ...
    [t.numberState addPrefix:@"0x" forRadix:16.0];
    tok = [t nextToken];
    TDEquals(183.0, tok.doubleValue);
    TDTrue(tok.isNumber);
    ...
    [t.numberState addPrefix:@"0x" forRadix:16.0];
    tok = [t nextToken];
    TDEquals(2286.0, tok.doubleValue);
    TDTrue(tok.isNumber);
