...
- (void)testNegatedPredicate {
    ...
    TDEqualObjects(TDAssembly(@"[0]NOT/0/</2^"), [_res description]);
    
    _res = [_parser parseString:@"! 0 < 2" error:&err];
    TDEqualObjects(TDAssembly(@"[0]!/0/</2^"), [_res description]);
}
...
//    TDEqualObjects(TDAssembly(@"[1]foo^"), [_res description]);
//    
//    t.string = @"bar";
//    a = [PKAssembly assemblyWithTokenizer:t];
//    res = [[eval.parser parserNamed:@"keyPath"] completeMatchFor:a];
//    TDEqualObjects(TDAssembly(@"[0]bar^"), [res description]);
//    
//    t.string = @"baz";
//    a = [PKAssembly assemblyWithTokenizer:t];
//    res = [[eval.parser parserNamed:@"keyPath"] completeMatchFor:a];
//    TDEqualObjects(TDAssembly(@"[0]baz^"), [res description]);
//    
//    t.string = @"foo.bar";
//    a = [PKAssembly assemblyWithTokenizer:t];
//    res = [[eval.parser parserNamed:@"keyPath"] completeMatchFor:a];
//    TDEqualObjects(TDAssembly(@"[0]foo.bar^"), [res description]);
//}
...
- (void)testStringTest {
    ...
    TDEqualObjects(TDAssembly(@"[1]'foo'/BEGINSWITH/'f'^"), [_res description]);
    
    _res = [_parser parseString:@"'foo' BEGINSWITH 'o'" error:&err];
    TDEqualObjects(TDAssembly(@"[0]'foo'/BEGINSWITH/'o'^"), [_res description]);
    
    _res = [_parser parseString:@"'foo' ENDSWITH 'f'" error:&err];
    TDEqualObjects(TDAssembly(@"[0]'foo'/ENDSWITH/'f'^"), [_res description]);
    
    _res = [_parser parseString:@"'foo' ENDSWITH 'o'" error:&err];
    TDEqualObjects(TDAssembly(@"[1]'foo'/ENDSWITH/'o'^"), [_res description]);
    
    _res = [_parser parseString:@"'foo' CONTAINS 'fo'" error:&err];
    TDEqualObjects(TDAssembly(@"[1]'foo'/CONTAINS/'fo'^"), [_res description]);
    
    _res = [_parser parseString:@"'foo' CONTAINS '-'" error:&err];
    TDEqualObjects(TDAssembly(@"[0]'foo'/CONTAINS/'-'^"), [_res description]);
}
...
- (void)testComparison {
    ...
    TDEqualObjects(TDAssembly(@"[1]1/</2^"), [_res description]);
    
    _res = [_parser parseString:@"1 > 2" error:&err];
    TDEqualObjects(TDAssembly(@"[0]1/>/2^"), [_res description]);
...
- (void)testTruePredicate {
    ...
    TDEqualObjects(TDAssembly(@"[1]TRUEPREDICATE^"), [_res description]);
}
...
- (void)testFalsePredicate {
    ...
    TDEqualObjects(TDAssembly(@"[0]FALSEPREDICATE^"), [_res description]);
}
...
- (void)testCollectionTest {
    ...
    TDEqualObjects(TDAssembly(@"[1]1/IN/{/1/}^"), [_res description]);
}
...
- (void)testCollectionLtComparison {
    ...
    TDEqualObjects(TDAssembly(@"[1]ANY/{/3/}/</4^"), [_res description]);
    
    _res = [_parser parseString:@"SOME {3} < 4" error:&err];
    TDEqualObjects(TDAssembly(@"[1]SOME/{/3/}/</4^"), [_res description]);
    
    _res = [_parser parseString:@"NONE {3} < 4" error:&err];
    TDEqualObjects(TDAssembly(@"[0]NONE/{/3/}/</4^"), [_res description]);
    
    _res = [_parser parseString:@"ALL {3} < 4" error:&err];
    TDEqualObjects(TDAssembly(@"[1]ALL/{/3/}/</4^"), [_res description]);
}
...
- (void)testCollectionGtComparison {
    NSError *err = nil;
    _res = [_parser parseString:@"ANY {3} > 4" error:&err];
    TDEqualObjects(TDAssembly(@"[0]ANY/{/3/}/>/4^"), [_res description]);
    
    _res = [_parser parseString:@"SOME {3} > 4" error:&err];
    TDEqualObjects(TDAssembly(@"[0]SOME/{/3/}/>/4^"), [_res description]);
    
    _res = [_parser parseString:@"NONE {3} > 4" error:&err];
    TDEqualObjects(TDAssembly(@"[1]NONE/{/3/}/>/4^"), [_res description]);
    
    _res = [_parser parseString:@"ALL {3} > 4" error:&err];
    TDEqualObjects(TDAssembly(@"[0]ALL/{/3/}/>/4^"), [_res description]);
}
...
- (void)testOr {
    ...
    TDEqualObjects(TDAssembly(@"[1]TRUEPREDICATE/OR/FALSEPREDICATE^"), [_res description]);
}
...
- (void)testAnd {
    ...
    TDEqualObjects(TDAssembly(@"[0]TRUEPREDICATE/AND/FALSEPREDICATE^"), [_res description]);
}
