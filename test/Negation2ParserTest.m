...
static Negation2Parser *parser;

@interface Negation2ParserTest : XCTestCase
@end
...
+ (void)setUp {
    ...
    NSError *err = nil;
    NSString *path = [[NSBundle bundleForClass:self] pathForResource:@"negation2" ofType:@"grammar"];
...
#if TD_EMIT
    path = [[NSString stringWithFormat:@"%s/test/Negation2Parser.h", getenv("PWD")] stringByExpandingTildeInPath];
    err = nil;
    if (![visitor.interfaceOutputString writeToFile:path atomically:YES encoding:NSUTF8StringEncoding error:&err]) {
        NSLog(@"%@", err);
    }

    path = [[NSString stringWithFormat:@"%s/test/Negation2Parser.m", getenv("PWD")] stringByExpandingTildeInPath];
    err = nil;
    ...
    parser = [[Negation2Parser alloc] initWithDelegate:nil];
}
